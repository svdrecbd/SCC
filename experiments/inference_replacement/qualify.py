"""Frozen-source Charon qualification, no training or dependencies beyond Python."""
...
cfg=json.loads((src/'config.json').read_text());out.mkdir(parents=True,exist_ok=False);start=time.monotonic()
(out/'machine.json').write_text(json.dumps(dict(host=platform.node(),platform=platform.platform(),
    python=sys.version,executable=sys.executable,cpu_count=os.cpu_count(),started=datetime.now(timezone.utc).isoformat()),indent=2)+'\n')
try:
    ...
    receipt=dict(validation='PASS',cases=len(rows),partition_queries=sum(r['query_count'] for r in summary),
                 perturbation_corners=len(rows)*len(cfg['perturbations'])*8,corruptions=len(negative),
                 repair_source_bytes=(src/'repair.py').stat().st_size,
