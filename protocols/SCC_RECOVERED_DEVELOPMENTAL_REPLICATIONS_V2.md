# Seed23 replication: engineering gate correction V2

2026-09-11. The seed23 attempt stopped before any experimental arm trained.
Its four-update engineering fixture had bitwise-identical model weights,
ordinary/episode chains and step counts. One Adam first-moment entry differed
by at most1.8189894e-12. Preserve that failed attempt and its raw states.

For this replacement attempt, extend each timing/method resume fixture to16
updates (split at8), and require floating model/optimizer tensors to agree
within1e-7 absolute error. Stream/RNG state and all non-floating configuration
remain exact. Report bitwise equality separately. This is the same numerical
precision criterion adopted for the SEAM supplement, not a changed training
objective, schedule, data, model or scientific gate. The already running
seed17 and41 experiments keep their original source and completed gates.

The inherited replication protocol follows. Only the seed23/data202 repetition
is relaunched under this engineering correction.

---

# Corrected developmental replications V1

2026-09-11, declared while the seed17 pilot is running and before its final
arm results have been retrieved. The user has removed the aggregate compute
limit. Run two independent repetitions of the same four-arm experiment:
(model initialization23, ordinary/episode data seed202) and (initialization41,
data seed303). In each repetition, all four arms share their initialization and
ordinary stream; early and late share episode samples, counts, density and
length. Keep every training/evaluation/repair hyperparameter unchanged. The
validation seed582019 and reader/calibration support seed829156 stay fixed for
paired comparisons. These are independent training runs on a shared benchmark,
not independent validation examples or separate text corpora.

Each repetition executes its actual CUDA readiness checks first. A failed
gate is an implementation outcome and must be preserved, not counted as a
scientific negative result. Replications do not receive adaptive extra training
or hyperparameter changes. Report all seeds and all arms, including failed
intact gates. Runtime/output limits stay75 H100 minutes and2GiB per repetition;
record measured wall time, storage and charges. The old aggregate$10 limit in
the inherited protocol is superseded by the user's explicit authorization.
This is open development replication, not a sealed confirmatory experiment.

The full inherited protocol follows. Its seed17/data101 description identifies
the original run; the two declared seed pairs above override only those values.
