...
def main():
    ...
    paths=sorted((ROOT/'scc').glob('*.py'))+[ROOT/n for n in ('pyproject.toml','uv.lock','.python-version','scripts/consolidated_replay_probe.py','protocols/SCC_CONSOLIDATED_REPLAY_ABLATION_V1.md')]
...
sys.argv=['consolidated_replay_probe.py','--output','/output/consolidated-replay','--data','/workspace/data','--parents','/tmp/parents.json','--device','cuda']
runpy.run_path('/workspace/scripts/consolidated_replay_probe.py',run_name='__main__')
'''
    ...
    request={'chip':'h100','chip_count':1,'context_id':'ctx-983c199b','command':'python -c '+shlex.quote(loader),
      ...
      'idempotency_key':a.key,'label':'SCC common benign consolidation and targeted probe: '+', '.join(label for spec in specs for label in spec['parents']),
      'resume':'none','max_restarts':0,'checks':{'success':{'check':{'type':'file_exists','path':'/output/consolidated-replay/result.json'}}}}
