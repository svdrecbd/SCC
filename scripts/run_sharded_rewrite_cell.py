"""LN-067 custom recurrent rewrite and declared graph-edit counterexamples."""
import argparse
import copy
import json
...
from scc.sharded_rewrite_cell import ShardedRewriteCell,repack_current
...
from scripts.rewrite_rank_certificate import certificate
...
CASES=('intact','identity','scale-negative','scale-half','scale-double','controller-edited',
       'symmetric-fixed-D','symmetric-normalizer','symmetric-compensated','identity-binding',
       'skip-commit','frozen-binding','symbolic-binding','direct-compilation',
       'same-capacity-repack','repair-after-merge','repair-after-erasure')
...
def intervene(model,name,proposal,warm_ids,rows,folder,fixture):
    if name.startswith('scale-'):model.rescale({'scale-negative':-1.,'scale-half':.5,'scale-double':2.}[name])
        ...
        if name=='identity-binding':model.storage_rule='identity'
        ...
        elif name=='same-capacity-repack':
            torch.save({'post_event_state':pack_state(model)},folder/(name+'-start.pt'))
            model=repack_current(model)
        elif name in ('repair-after-merge','repair-after-erasure'):
            if name=='repair-after-merge':model.recode(1.,change_normalizer=True,change_writer=True)
            ...
            model=repack_current(model,direct=True)
    return model


def freeze(folder,fixture):
    text=(ROOT/'labnotes.md').read_text().split('<a id="ln-067"></a>')[1].split('\n<a id=')[0].split('\n## Supporting-record')[0]
    (folder/'plan.md').write_text('<a id="ln-067"></a>'+text)
    manifest=snapshot_sources(folder/'source')
    for name in ('scripts/run_sharded_rewrite_cell.py','scripts/audit_sharded_rewrite_cell.py',
                 'scripts/audit_rewrite_binding_cell.py','scripts/rewrite_rank_certificate.py',
                 'scripts/run_learned_binding_bank.py','scripts/localize_persistent_learning.py',
                 'tests/test_sharded_rewrite_cell.py'):
    ...
    config={'fixture':fixture,'cases':CASES,'width':128,'streams':4,'warmup_seed':17313014,
        'evaluation_seed':17313015,'warmup_per_cell':16,'evaluation_per_cell':128,
        'precisions':['fp32','fp64'],'cpu_threads':2,'wall_seconds':120,'output_limit_bytes':180*1024**2,
        'training_updates':0,'optimizer_repair_updates':0,'storage_sectors':8,'payload_shards':2,'parameter_padding':1,
        'parameter_commits_per_request':1,'hidden_commits_per_request':19,
        'phase_rule':'token_id modulo 2; swap sectors 1/2 and 5/6 for odd tokens',
...
def main():
    ...
    def timeout(*_):raise TimeoutError('LN-067 120-second wall limit')
    ...
    try:
        ...
        atomic_json(folder/'rank-certificate.json',certificate())
        ...
        torch.save({'intact':full,'controller_edit':proposal},folder/'proposals.pt')
        warm,rows=data(17313014,16),data(17313015,128)
        ...
        for precision,dtype in (('fp32',torch.float32),('fp64',torch.float64)):
            ...
            reference=RewriteBindingCell(full.to(dtype),width,4);reference_stream(reference,warm_ids)
            ref=reference_stream(reference,ids);torch.save(ref,dest/'reference.pt')
            m=ShardedRewriteCell(full.to(dtype),width,4)
        ...
        for precision,dtype in (('fp32',torch.float32),('fp64',torch.float64)):
            for name in CASES[1:]:
                ...
                m=intervene(m,name,proposal.to(dtype),warm_ids,rows,folder/precision,args.fixture)
                ...
                result={'precision':precision,'case':name,'scores':scores,'first_erasure':m.first_erasure.tolist(),
                    ...
                    'stored_parameter_state_scalars':m.bank.numel()+m.hidden_bank.numel(),
