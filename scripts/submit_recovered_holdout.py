...
def main():
    ...
    paths=sorted((ROOT/'scc').glob('*.py'))+[ROOT/n for n in ('pyproject.toml','uv.lock','.python-version','scripts/evaluate_recovered_holdout.py','protocols/SCC_RECOVERED_HELDOUT_CONFIRMATION_V1.md')]
...
sys.argv=['evaluate_recovered_holdout.py','--output','/output/heldout','--data','/workspace/data','--parents','/tmp/parents.json','--device','cuda']
runpy.run_path('/workspace/scripts/evaluate_recovered_holdout.py',run_name='__main__')
'''
    ...
    request={'chip':'h100','chip_count':1,'context_id':'ctx-983c199b','command':'python -c '+shlex.quote(loader),
      ...
      'resume':'none','max_restarts':0,'checks':{'success':{'check':{'type':'file_exists','path':'/output/heldout/result.json'}}}}
