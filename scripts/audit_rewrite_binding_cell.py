"""Independent scoring and scalar graph/GRU reconstruction for LN-064 artifacts."""
import argparse
import json
import math
...
def operator(decisions,odd,rule,sign,dtype):
    # Independent union-find implementation, rather than the runtime closure.
    owners=[0,1,0,1] if odd else [0,0,1,1]
    d=torch.eye(4,dtype=dtype)*.5
    for a,b in ([(0,1),(2,3)] if odd else [(0,2),(1,3)]):d[a,b]=d[b,a]=sign*.5
    if rule=='identity':p=torch.eye(4,dtype=dtype)
    elif rule=='frozen':p=torch.tensor([[.5,.5,0,0],[.5,.5,0,0],[0,0,.5,.5],[0,0,.5,.5]],dtype=dtype)
    else:
        parent=list(range(4))
        def find(i):
        ...
        for a in range(4):
            for b in range(4):
                allowed=(owners[a]==owners[b]) if rule=='symbolic' else bool(decisions[owners[a]*2+owners[b]])
                if allowed:parent[find(a)]=find(b)
        p=torch.zeros(4,4,dtype=dtype)
        for a in range(4):
            group=[b for b in range(4) if find(b)==find(a)]
...
def audit_trace(saved,initial,out,ids):
    ...
    decisions=out['admitted'][:,0]
    for i in range(len(params)):
        op=operator(decisions[i],False,initial['storage_rule'],initial['normalizer_sign'],dtype)
    ...
    weights=[unpack_parameters(params[i,0,:task_count]*gain,width) for i in range(len(params))]
    previous=initial['hidden_bank'][:,0]*gain
    ...
    for tick in first['ticks']:
        ...
        for i,w in enumerate(weights):
            ...
            op=operator(decisions[i],odd,initial['storage_rule'],initial['normalizer_sign'],dtype)
            ...
            if initial['execution']=='commit':expected=op@scratch
                ...
                expected=(basis[:,None]*candidate[None,:]/gain)[order]
            torch.testing.assert_close(expected,tick['committed'][i],rtol=0,atol=tolerance)
            torch.testing.assert_close(tick['committed'][i,0]*gain,tick['after'][i],rtol=0,atol=0)
            replayed+=1
...
def audit(folder):
    ...
    for r in summary['results']:
        ...
        if name in ('controller-silenced','identity-binding','skip-commit'):
            assert (out['active_state_changes']==0).all() and torch.count_nonzero(final_bank)>0
        if name.startswith('repair-after-'):
            start=torch.load(dest/(name+'-start.pt'),map_location='cpu',weights_only=True)['post_event_state']
            assert torch.equal(bank(start)[:,0]*start['gain'],bank(initial)[:,0]*initial['gain'])
            assert torch.equal(start['hidden_bank'][:,0]*start['gain'],initial['hidden_bank'][:,0]*initial['gain'])
