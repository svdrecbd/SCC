"""Submit the frozen held-out evaluation against hash-identified GMAN endpoints."""
...
def main():
    ...
    paths=sorted((ROOT/'scc').glob('*.py'))+[ROOT/n for n in ('pyproject.toml','uv.lock','.python-version','scripts/evaluate_consolidated_holdout.py','protocols/SCC_ALIGNMENT_CONSOLIDATED_HELDOUT_V1.md')]
    entries=[(path,str(path.relative_to(ROOT))) for path in paths]
    # The reusable training image intentionally excludes the test partition.
    # Upload these exact hash-verified files only for this frozen evaluation.
    entries += [(path,'data/'+path.name) for path in sorted((ROOT/'artifacts/retrieval-recovery/byte-prepared').glob('test.*.bin'))]
    assert len(entries)==len(paths)+3
...
sys.argv=['evaluate_consolidated_holdout.py','--output','/output/consolidated-heldout','--data','/workspace/data','--parents','/tmp/parents.json','--device','cuda']
runpy.run_path('/workspace/scripts/evaluate_consolidated_holdout.py',run_name='__main__')
'''
    ...
    request={'chip':'h100','chip_count':1,'context_id':'ctx-983c199b','command':'python -c '+shlex.quote(loader),
      ...
      'idempotency_key':a.key,'label':'SCC frozen held-out evaluation: '+', '.join(label for spec in specs for label in spec['parents']),
      'resume':'none','max_restarts':0,'checks':{'success':{'check':{'type':'file_exists','path':'/output/consolidated-heldout/result.json'}}}}
