...
def original_ids(family,fi):
    seen=set();attempt=0
    while len(seen)<512:
        row=make_row(random.Random(492085171+fi*1000000+attempt),family,'ungated','test');attempt+=1
...
def rows_for_layout(reordered):
    ...
    for fi,family in enumerate(FAMILIES):
        excluded=original_ids(family,fi);seen=set();attempt=0
        while len(seen)<256:
            if attempt>100000:raise RuntimeError('Finite complement exhausted')
            seed=775030291+fi*1000000+attempt;attempt+=1
...
def measure(model,bank,test,indices,rows,output,device):
    ...
    result={'branches':[{'reader':{'name':'identity','sign':1,'digit_sources':list(range(10))},'temperature':chosen,'support_temperature_nll':losses,'layouts':layouts}],
        'split':'test','subset':'untouched_complement_v1','training':False,'source_fitting_split':'train'}
...
def main():
    ...
    protocol=pathlib.Path(__file__).resolve().parents[1]/'protocols/SCC_ALIGNMENT_CONSOLIDATED_HELDOUT_V1.md';shutil.copyfile(protocol,a.output/'protocol.md')
    ...
    for source in TEXT_SOURCES:
        ...
        original=pool[torch.randperm(len(pool),generator=torch.Generator().manual_seed(92031+group))[:256]]
        pool=pool[~torch.isin(pool,original)];assert len(pool)>=256
        indices[source]=pool[torch.randperm(len(pool),generator=torch.Generator().manual_seed(992033+group))[:256]]
        assert not torch.isin(indices[source],original).any()
