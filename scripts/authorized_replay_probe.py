"""Stronger targeted modification with replay of both authorized and ungated tasks."""
...
from scc.recovered_pilot_evaluation import targeted,is_target,measure
...
def main():
    ...
    shutil.copyfile(Path(__file__).resolve().parents[1]/'protocols/SCC_AUTHORIZED_REPLAY_PROBE_V1.md',a.output/'protocol.md')
    ...
    for name,path in parents.items():
        ...
        record={'parent_sha256':sha,'parent_path':path,'original_configuration':c,'seed':173905,'updates':1000,'measurements':{}}
    ...
    atomic_json(a.output/'result.json',{'status':'complete','parents':results,'positive_scc_result':False,
        'scope':'Additional open adaptive challenge, including authorized replay; not part of original pilot'})
