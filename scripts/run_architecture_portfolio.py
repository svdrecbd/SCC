"""Train and challenge one frozen learned construction; never polls GPU jobs."""
import argparse
import collections
import copy
...
from scc.portfolio_models import PortfolioModel, PortfolioConfig, variant_config, from_checkpoint, VARIANTS
...
PROTOCOL = 'SCC_ARCHITECTURE_PORTFOLIO_V1.md'


def configuration(variant, mode, seed, smoke=False):
    return {'architecture': 'scc-portfolio/v1', 'model': asdict(variant_config(variant, smoke)),
            'variant': variant, 'mode': mode, 'seed': seed, 'data_seed': 101,
            ...
            'evidence_class': 'CPU implementation fixture' if smoke else 'Open architecture/construction screening; no sealed test'}
...
def train(bank, config, folder, device, deadline):
    ...
    model = PortfolioModel(PortfolioConfig(**config['model'])).to(device).train()
    initial_state_sha256 = fingerprint(model)
...
def probe(parent, bank, config, folder, scope, benign, deadline, *, edit_spec=None):
    folder.mkdir()
    started = time.monotonic()
    model = copy.deepcopy(parent).train()
    spec = {'seed':193905, 'lr':1e-4} if edit_spec is None else dict(edit_spec)
    if set(spec) != {'seed','lr'}: raise ValueError('Declare only edit seed and learning rate')
    stream = EditStepper(model, bank, batch_size=config['edit_batch_size'], scope=scope, benign=benign, **spec)
    panel = panels(2 if config['smoke'] else 32, 3 if config['smoke'] else 64, config['evaluation_seed'])
    ...
    with (folder / 'curve.jsonl').open('w') as log, (folder / 'edit-steps.jsonl').open('w') as edits:
        for step in range(config['edit_steps'] + 1):
            ...
            if step:
                ...
                update['elapsed_seconds'] = time.monotonic() - update_start
    ...
    result = {'scope': scope, 'benign': benign, 'completed_modification_steps': completed,
              'edit_specification':spec, 'repair_learning_rate':1e-4,
    ...
    with (folder / 'repair-steps.jsonl').open('w') as log:
        for step in range(config['edit_steps']):
            ...
            record['elapsed_seconds'] = time.monotonic() - update_start
            log.write(json.dumps(record) + '\n')
            log.flush()
...
def run(a):
    ...
    result = {'schema': 'architecture-portfolio/v1', 'configuration': config, 'environment': environment(device),
    ...
    if qualified or a.smoke:
        for label, scope, benign in [('core', 'core', False), ('all', 'all', False), ('benign', 'all', True)]:
            ...
            atomic_json(a.output / 'progress.json', result)
        # A graph substitution is a separate, explicitly broader editing challenge.
        if time.monotonic() < started + 6900:
    ...
    assert size < 1024 ** 3
    complete = training['complete'] and (not qualified and not a.smoke or len(result['probes']) == 3 and all(r['status'] == 'complete' for r in result['probes'].values()) and 'graph_bypass' in result)
