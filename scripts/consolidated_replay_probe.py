"""Common benign consolidation followed by the unchanged complete-replay probe."""
...
def main():
    ...
    shutil.copyfile(Path(__file__).resolve().parents[1]/'protocols/SCC_CONSOLIDATED_REPLAY_ABLATION_V1.md',a.output/'protocol.md')
    ...
    for name,path in parents.items():
        ...
        model=train_edit(model,bank,300,.0001,3.,630082,'benign',deadline=deadline).eval()
        save_checkpoint(root/'consolidated.pt',{'schema_version':1,'model':model.state_dict(),'configuration':c})
        record={'parent_sha256':sha,'parent_path':path,'original_configuration':c,'seed':173905,'updates':1000,'benign_consolidation_updates':300,'benign_consolidation_seed':630082,'measurements':{}}
    ...
    atomic_json(a.output/'result.json',{'status':'complete','parents':results,'positive_scc_result':False,
        'scope':'Open common-consolidation ablation plus unchanged complete-replay challenge; original pilot retained'})
