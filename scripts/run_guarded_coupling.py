"""Paired generated-answer guard experiment; immutable bounded jobs, no polling."""
...
def run(args):
    ...
    frozen = {p: file_digest(ROOT/p) for p in HELPERS + ['scripts/run_guarded_coupling.py', 'protocols/'+PROTOCOL]}
    ...
    config.update(evaluation_size=4 if args.smoke else 128, text_blocks=2 if args.smoke else 128,
                  ...
                  continuation='generation-guard/v1', guard_mode=args.guard, smoke=args.smoke)
    ...
    result = {'status': 'running', 'configuration': config, 'environment': environment(device),
              ...
              'scope': args.scope, 'repair_scope': 'all', 'seed': args.seed, 'guard_mode':args.guard}
    ...
    if selected is not None:
        ...
        fixed_rows,guard_ids = guard_rows(1021731,2 if args.smoke else 128,fit_reserved['excluded_task_ids'])
        fit_reserved['excluded_task_ids'].update(guard_ids)
        guard = GenerationGuard(model,args.output/'generation-guard',fixed_rows,
                                fit_reserved['excluded_task_ids'],fresh_size=2 if args.smoke else 32)
        ...
        with (args.output/'training.jsonl').open('w') as log:
            ...
            fitted, fitting = optimize_varied(model, selected, episode_factory, fixed, anchor_factory,
                                              ...
                                              trial_guard=guard,enforce_guard=args.guard=='generation')
        ...
        training_used['excluded_task_ids'].update(guard.used_ids)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--guard',choices=('generation','observe'),required=True)
