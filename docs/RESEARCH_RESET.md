...
```mermaid
flowchart LR
    A[Useful intact model] --> B[Protected function removed]
    B --> C[Indispensable computation damaged]
    C --> D[Abilities fail on fresh inputs]
    D --> E[Failure persists within the stated boundary]
```
